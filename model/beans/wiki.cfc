...
<cfscript>
	public any function init(required string ContentID, required string SiteID, beanFactory) {
		...
		if (Wiki.getUseIndex() && Wiki.getIsInit()) {
			...
			doIndex(collection='Murawiki_#Wiki.getContentID()#',query=allPages,key='Label',title='Title',body='Body');
		}
		// }}}
	...
	public string function renderHTML(required any wikiPage, required any ContRend) {
		var wiki = getWiki(ARGUMENTS.wikiPage.getParentID());
		return wiki.engine.renderHTML( ARGUMENTS.wikiPage.getBlurb(), ListLast(ARGUMENTS.wikiPage.getFilename(), '/'), Wiki.getWikiList(), wiki.getFileName(), ContRend ).blurb;
	}
